...
공통 기간과 검증 통과 여부를 읽고, Supabase finance 장부의
두 종목 행 수·기간과 서로 맞는지 대조해줘.
...
손으로 확인이 끝나면 통과를 기록합니다.

```text
방금 사이클 손 검산이 스크립트와 일치했어.
같은 조건으로 평가 구간 보고서를 hand-check 통과 상태로 다시 저장해줘.
```
...
경고가 남아 있으면 그 경고가 왜 남았는지부터 확인합니다.
경고를 지우려고 값을 바꾸지 않습니다.
