# PROMPTS — 에이전트에게 보내는 메시지 모음

자산운용팀 실습에서 에이전트에게 보내는 자연어 메시지를 작업별로 모았습니다. 유닛별로 나눠 두었고, 각 항목에 누구에게 보내는지 적어 두었습니다.
코드 블록을 통째로 복사해 대화창에 붙여넣으세요. 괄호 자리만 본인 상황에 맞게 바꿉니다.

> 8.1은 키움 공식 `kwcli 1.0.0`·OS Keyring·새 Sam 세션 preflight 실측을 반영했습니다.

# 헤르메스에 증권 오픈API 연동하기 (8.1)

## 1. 자산운용팀 개소 — 원칙 세우기 (Sam)

```text
오늘부터 자산운용팀을 개소한다. 너는 개발 담당으로 파견이야.
이 팀의 원칙 세 가지를 먼저 기억해줘.
1) 모든 거래는 모의투자 연습 계좌에서만 한다
2) 주문은 어떤 경우에도 내 승인 없이 실행하지 않는다
3) 앱키와 계좌 정보는 화면에 출력하지 않는다
확인했으면 원칙 세 가지를 한 줄씩 요약해서 답해줘.
```

원칙을 먼저 문장으로 박아두는 것이 이 섹션의 안전장치입니다.
본인 업무에 쓸 때는 "돈·삭제·발송처럼 되돌리기 어려운 일"에 같은 패턴을 쓰세요.

## 2. 한도 템플릿을 수정하고 확정한다 (사람이 직접)

에이전트에게 보내는 메시지가 아닙니다. 스타터에 들어 있는 `guardrails/limits.md`를 열어 **여러분의 상황에 맞게 직접 수정하세요.**

```markdown
---
status: confirmed
updated_by: human
---

# 투자 한도

- 시드머니: 300만원 (모의)
- 1회 주문 한도: 최대 2주
- 1일 판단·주문 기안: 1회 (같은 날 재실행은 판정하지 않음)
- 보유 수량 상한: 10주
- 손실 한도: -10% 도달 시 전체 중단
- 매수 금지: 장 미개장 · 예산 소진 · 보유 상한 도달
- 오류 시: 재시도하지 않는다. 보고하고 멈춘다
```

위 숫자는 모의투자용 초기값입니다. 가용 예산과 감당 가능한 손실 범위에 맞게 바꾸고, 모든 항목을 확인한 뒤에만 `status: unconfirmed`를 `status: confirmed`로 변경합니다.

자격 증명 등록보다 먼저 하는 이유가 있습니다. 외부 권한을 먼저 연결하고 안전장치를 나중에 붙이면 그건 옵션처럼 보입니다. **에이전트가 자기 한도를 정하거나 확정하게 하지 않습니다.**

## 3. 새 Sam 세션 preflight

```text
읽기 전용 환경 점검만 해줘.
1) command -v kiwoomcli
2) kiwoomcli auth status --profile 모의계좌
3) kiwoomcli doctor
자격 증명·토큰·계좌번호 원문은 출력하지 마. `command -v` 결과로 PATH 인식 여부를 적고, `auth status` 출력의 `계좌 별칭`, `모드`, `자격 증명 존재`, `자격 증명 출처`, `토큰 유효`, `지금 API 호출 가능` 값을 해석하거나 번역하지 말고 그대로 옮겨줘. 값이 모순되면 통과로 판정하지 마.
```

다섯 항목을 모두 통과한 뒤에만 조회합니다.

## 4. 연동과 첫 조회 (Sam)

```text
kiwoom-broker 스킬과 키움 공식 CLI의 `모의계좌` 프로필을 사용해서

1) KODEX 200(069500)의 종목 정보를 조회하고
2) 모의투자 계좌 잔고를 조회해서 보여줘.

접근토큰·앱키·시크릿·계좌번호 값은 출력하지 마. 주문은 오늘 하지 않는다.
```

마지막 두 문장이 핵심입니다. **비노출**과 **범위 제한**(오늘은 조회까지)을
요청 문장에 함께 넣는 패턴은 어떤 API 연동에든 그대로 옮겨 쓸 수 있습니다.

> 💡 잔고에 "모의투자 해당조회내역이 없습니다"가 나오면 **정상**입니다.
> 오류가 아니라 아직 아무것도 사지 않았다는 뜻입니다.

## 5. 역할과 실행 경계 (개념 — 8.4에서 실제로 씀)

Keyring은 OS 사용자 단위라서 같은 사용자의 Ada와 Sam을 물리적으로 격리하는 보안 경계가 아닙니다. 이 프로젝트에서는 `kiwoom-broker` 스킬을 Sam에게 맡기고, Ada는 판단 파일만 작성하며, 주문은 `OrderIntent → ApprovalRecord → --confirm`을 통과하게 해 역할을 나눉니다. 실계좌라면 별도 OS 사용자·컨테이너·실행 정책이 필요합니다.

---

# 투자 DB 설계와 수집 (8.2)

실행 순서는 **0 → 1 → 2 → 3 → 4 → 5 → 6 → 7 → 8**입니다. 먼저 8.1의 주문
프로젝트가 현재 경로에 있는지 확인하고, 분석 도구 설치가 끝나면 같은 경로에서 세
프로필을 시작해 데이터베이스·카드를 차례로 구성합니다.
같은 Sam 카드에서 수집 범위를 확인·승인하고, 품질 검증이 끝난 뒤 Supabase 적재
범위를 승인합니다.

## 0. 주문 프로젝트 재료 점검 (Ada)

8.1에서 만든 주문 프로젝트 폴더에서 세션을 시작합니다.

```bash
cd "$HOME/.hermes/workspace/magma-finance-lab"
```

Sam·Oliver·Ada 세션은 모두 이 폴더에서 시작합니다. 오늘 쓸 재료가 제자리에
있는지는 터미널 대신 Ada에게 자연어로 확인을 맡깁니다.

```text
오늘 작업에 필요한 재료가 이 폴더에 있는지 확인해서 보고해줘.
장기 일봉을 수집할 스크립트, Supabase core 마이그레이션 SQL,
setup이 남긴 vibe-finance-kit 설치 증거 파일 세 가지야.
설치 증거에서는 doctor 결과와 고정 샘플 검증 결과만 요약해줘.
비밀값은 출력하지 마.
```

칸반을 실행하기 전에는 활성 보드, 잔존 카드, Sam Gateway heartbeat, 세 프로필
인증 상태도 확인합니다.

## 1. Skill과 MCP 경계 확인 (Ada·Oliver)

```text
vibe-finance-kit의 finance_kit_doctor를 실제로 호출해줘.
read_only, order_tools, broker_credentials_required만 보여줘.
```

Vibe Finance Kit을 처음 설치할 때는 macOS, Linux·WSL, Windows PowerShell에서 같은
명령을 사용합니다.

```text
cd "$HOME/.hermes/workspace"
git clone https://github.com/dandacompany/vibe-finance-kit.git
cd vibe-finance-kit
uv run python scripts/setup_hermes.py
```

마지막 명령이 프로젝트 환경, Ada Skill 3개, Oliver Skill 2개, Ada MCP를 한 번에
설정합니다. MCP 도구 4개 활성화 질문이 나오면 `Y`를 입력합니다.

Oliver의 새 세션에서는 공개자료 조사 Skill 두 개가 보이고 주문·브로커·계좌 도구가 없는지 확인합니다.

```text
현재 설치된 finance-research-discipline과 etf-value-analysis Skill을 확인해줘.
주문·브로커·계좌 도구가 있다면 이름을 보고하고, 없다면 없다고 답해줘.
```

Ada 새 세션에서 고정 샘플을 먼저 검증합니다.

```text
$HOME/.hermes/workspace/vibe-finance-kit/examples/etf-analysis-snapshot.json을 읽고
validate_etf_snapshot 도구를 실제로 호출해줘.
valid, errors, warnings, order_eligible만 보여줘.
```

설치 증거 파일 `artifacts/setup/setup-receipt-vibe-finance-kit.json`은 setup의
마지막 단계가 자동으로 작성합니다. 별도로 만들 필요 없이 0번 재료 점검에서 doctor
결과와 고정 샘플 검증 결과를 요약해 확인합니다. 키·토큰·계좌번호는 기록되지 않습니다.

이어서 새 Sam 세션에서 키움 연결을 확인하고 결과를 증거 파일로 남깁니다.

```text
다음 세 명령을 실행하고 명령 경로, 모의계좌 인증 상태, doctor 결과만 요약해줘.
비밀값과 계좌번호는 출력하지 마.

command -v kiwoomcli
kiwoomcli auth status --profile 모의계좌
kiwoomcli doctor
```

```text
방금 확인한 키움 연결 결과를 contracts/broker-capabilities.schema.json에 맞춰
artifacts/setup/broker-capabilities.json으로 저장해줘.
profile은 모의계좌, mode는 demo로 기록해. quote_read, daily_candles_read,
balance_read는 이번 preflight에서 실제로 확인한 범위만 true로 기록하고
order_enabled는 false로 유지해. 키·시크릿·토큰·계좌번호는 저장하지 마.
저장 뒤 스키마를 다시 검증해줘.
```

## 2. 데이터 계약 검토 (Ada)

```text
이 저장소의 contracts/와 다음 두 파일을 읽어줘.
- supabase/migrations/20260806090000_finance_core.sql
- supabase/migrations/20260806091000_finance_analysis_contract.sql

라이브 데이터베이스에는 적용하지 말고 다음만 표로 검토해줘.
1) 종목코드가 A 접두 없는 6자리로 제한되는지
2) 일봉 중복을 어떤 키로 막는지
3) as_of와 available_at이 구분되는지
4) 결측을 null과 warnings로 보존하는지
5) Data API grant와 RLS가 각각 어떻게 제한되는지
```

Ada의 Supabase MCP로 현재 연결된 프로젝트 이름과 project ref, `finance` 스키마의
테이블 목록과 RLS 상태를 조회합니다. 비밀값은 출력하지 않습니다. 핵심 4개 테이블이
없으면 `finance_core.sql`의 생성 객체와 권한 변경만 먼저 보고하게 합니다.

```text
현재 연결된 Supabase 프로젝트의 이름과 project ref를 확인하고
finance 스키마의 symbols, daily_prices, positions, orders 존재 여부와
각 테이블의 RLS 상태를 확인해줘. 아직 변경하지 마.
비밀값과 연결 문자열은 출력하지 마.
```

핵심 4개 테이블이 없으면 core SQL의 적용 계획만 먼저 받습니다.

```text
supabase 마이그레이션 폴더에서 finance core 마이그레이션 SQL을 찾아 검토해줘.
생성할 스키마, 테이블, 인덱스, RLS 변경, 권한 변경을 항목별로 보고해줘.
아직 실행하지 말고 finance 이외의 객체가 포함되어 있으면 중단해.
분석 계약 마이그레이션은 검토 대상에서 제외해.
```

대상 project ref와 생성 객체를 확인한 뒤에만 다음 요청으로 core SQL 실행을 승인합니다.

```text
확인한 core SQL만 실행해줘.
완료 후 symbols, daily_prices, positions, orders와 RLS 상태를 다시 조회해줘.
분석 테이블 SQL은 실행하지 마.
```

## 3. Sam 수집 카드에 넣을 장기 일봉 요청

아래 요청은 5절에서 만드는 Sam 수집 카드의 본문입니다. 같은 요청을 별도 세션과
칸반 카드에서 각각 실행하지 않습니다. 두 ETF의 수정주가 확정 일봉을 각각 최대
3,000개 수집하며, 당일 봉은 제외하고 공통 거래일이 2,500개 미만이면 성공 처리하지 않습니다.

수집 범위와 성공 기준은 5절에서 만드는 Sam 카드 본문에 미리 기록합니다.
카드가 dispatch되면 별도 승인 대화 없이 실행되며, 완료 후 카드 로그와 산출물에서
결과를 확인합니다.

출력의 `gate_passed=true`, 두 종목의 `adjusted=true`, `common_bars`와
`common_start`·`common_end`를 확인합니다. 생성된 원천 스냅샷과 SQL은
`artifacts/market/`에만 남고 Git에는 포함되지 않습니다.

## 4. Supabase에 멱등 적재하고 검산하기 (Ada)

Sam 수집 카드와 Ada 품질 검증 카드가 끝난 뒤 진행합니다. 적재 승인 근거는 품질
카드의 완료 코멘트(품질 규칙 통과·종목별 행 수·공통 거래일·오류 0건)입니다.

칸반 대시보드에서 Ada 품질 카드를 열어 아래 승인 댓글을 입력하고, 카드 상태를
`ready`로 바꾼 뒤 dispatch합니다. 워커가 카드를 다시 실행하며 최신 댓글을 지시로
읽습니다.

```text
대표 적재 승인.
품질 검증을 통과한 Supabase 적재용 SQL을 finance 스키마에만 적재해줘.
적재 후 종목별 행 수·날짜 범위·공통 거래일을 확인하고,
중복 거래일·비정상 가격·미래 날짜가 0건인지 검산해 완료 코멘트로 남겨줘.
비밀값과 연결 문자열은 출력하지 마.
```

비대화식 워커에서 데이터베이스 쓰기가 막혀 카드가 blocked로 남으면, Ada 대화
세션에서 같은 승인 문구로 적재하는 폴백 경로를 사용합니다.

2,500개는 모든 투자 연구에 통용되는 절대 기준이 아닙니다. 이 실습의 일봉
분할매수 전략에서 약 10년을 확보해 보정 구간과 평가 구간을 시간순으로 나누고,
서로 다른 시장 국면을 포함시키기 위한 최소 게이트입니다.

## 5. 실제 수집 카드 만들기 (Sam·Oliver)

보드 개설과 카드 생성은 Sam 세션에서 진행합니다.

```text
현재 보드와 남아 있는 카드를 확인해줘.
'자산운용팀 시세 수집'이라는 이름으로 새 보드를 만들고, 모든 카드의 workdir을
$HOME/.hermes/workspace/magma-finance-lab으로 고정해줘.
카드를 만들기 전에 Sam Gateway heartbeat와 Sam·Oliver·Ada의 현재 인증 상태를 보고해줘.
```

```text
자산운용팀 시세 수집 보드에 다음 두 카드를 만들어줘.

1) Sam: KODEX 200(069500)과 TIGER 200(102110)의 수정주가 확정 일봉을
   최대 5페이지·종목별 최대 3,000봉으로 순차 수집한다.
   호출 간격은 1초 이상, scripts/backfill_prices.py를 사용하고
   artifacts/market/에 저장한다. 성공 기준은 두 종목 공통 거래일
   2,500개 이상으로 카드에 기록한다. 모의계좌 프로필만 사용한다.
2) Oliver: 두 ETF의 운용사·거래소 공식 공개자료에서 상품 구조, 총보수,
   추적오차·괴리율·순자산·기초지수 가치지표의 출처와 기준일을 조사하고
   artifacts/research/etf-product-sources.json에 저장한다.

두 카드 모두 아직 실행하지 말고 카드 ID와 assignee, workdir, 출력 경로만 보여줘.
```

보드에 표시된 실제 카드 ID와 의존성을 확인합니다. Sam·Oliver 카드를
`ready`로 전환하고 dispatch한 뒤 실제 상태가 `in_progress`로 바뀌는지 확인합니다.

선행 카드 완료만 기다리는 상황은 `dependency`입니다. 반면 미확정봉처럼
사람의 교정 입력이 필요한 실패는 `needs_input`, 필요한 조회 기능이나 자격 증명이
없는 실패는 `capability`로 분류해야 카드가 `blocked`에 남습니다.

## 6. 품질 게이트 연결 (Ada)

```text
Ada 품질 검증 카드를 추가해줘.
Sam의 artifacts/market/ 결과와
Oliver의 artifacts/research/etf-product-sources.json을 입력 절대경로로 기록하고,
두 카드가 완료된 뒤에만 시작하도록 dependency를 연결해.
공통 거래일 2,500개 이상, 중복 거래일, OHLC 관계, 수정주가 여부,
미래 날짜를 검증하도록 적어줘. 아직 실행하지 마.
```

검증 fixture는 계약 형식과 실패 분류를 연습하기 위한 자료입니다.
장기 백테스트에는 이 fixture가 아니라 앞에서 수집한 공통 장기 일봉을 사용합니다.

## 7. 이번 실행의 분석 스냅샷 생성·검증 (Ada)

분석 스냅샷 생성은 품질 카드에 dependency를 건 카드로 처리합니다. 칸반 대시보드에서
직접 만들거나 Sam 세션에 아래를 입력합니다. 대시보드에서 만들 때는 제목을 별도로
입력하고 workspace를 dir(공통 작업 폴더)로 지정합니다 — worktree로 만들면 산출물이
`.worktrees/<카드ID>/` 격리 사본에 남습니다.

```text
자산운용팀 시세 수집 보드에 Ada 분석 스냅샷 카드를 추가해줘.
품질 검증 카드가 완료된 뒤에만 시작하도록 dependency를 연결하고 바로 실행해줘.

카드 내용: 이번에 수집한 KODEX 200 MarketSnapshot과
Oliver가 저장한 ETF 공식 출처 조사 결과를 사용해서
ETFAnalysisSnapshot 계약에 맞는 JSON을 만든다.
가격 신호는 이번 확정 일봉에서 계산하고, 상품 품질과 기초지수 가치지표는
Oliver가 확인한 출처와 기준일만 사용한다. 같은 기준일에 확보하지 못한 값은
null로 두고 missing_fields와 warnings에 사유를 남긴다.
주문 가능 여부는 false로 유지하고 artifacts/analysis/ 아래에 저장한다.
비밀값과 계좌번호는 출력하지 않는다.
```

새 파일의 입력 출처와 기준 시각을 확인한 뒤 읽기 전용 도구로 검증합니다.

```text
이 프로젝트의 분석 산출물 폴더에서 KODEX 200 ETF 분석 스냅샷 JSON을 찾아
절대경로를 먼저 확인해줘. 예제 파일이 아니라 이번 실행에서 생성된 산출물이어야 해.
그 JSON이 ETFAnalysisSnapshot 계약을 통과하는지 읽기 전용 도구로 검증해줘.
값을 추정하거나 null을 다른 숫자로 대체하지 말고,
찾은 파일의 절대경로와 valid, errors, warnings, order_eligible만 반환해줘.
```

로컬에서도 같은 content hash를 확인할 수 있습니다.

```bash
python3 scripts/validate_artifact.py artifacts/analysis/etf-analysis-snapshot-069500.json
```

`examples/etf-analysis-snapshot.json`은 계약 확인용 예시입니다. 이번 실행 파일을 만들지
못했을 때 예시를 새 산출물처럼 말하지 않습니다.

현재 가치지표 스냅샷을 과거 시점의 매매 신호로 사용하지 않습니다. 과거 발표
시점이 확인된 가치지표 시계열이 없기 때문입니다. 8.3에서는 장기 가격 이력으로
규칙을 검증하고, 이 스냅샷은 상품 구조와 결과의 한계를 설명하는 근거로 사용합니다.

## 8. 매일 확정봉 수집 예약 (Sam)

먼저 Sam Gateway heartbeat가 정상인지 확인합니다. stale이면 cron을 만들지 않고
Gateway를 복구한 뒤 다시 확인합니다.

```text
평일 18시 30분에 실행되는 Hermes cron을 만들어줘.
profile은 sam, workdir은 $HOME/.hermes/workspace/magma-finance-lab으로 고정해.
KODEX 200(069500)과 TIGER 200(102110)의 직전 확정 일봉을 순차 수집하고,
Sam 수집이 끝나면 Ada 가격 품질 검증 카드를 실행해.
Oliver 공개자료는 이 매일 cron에 포함하지 마.
등록 뒤 cron ID, 일정, 다음 실행 시각, profile, workdir만 보여줘.
```

지속 운영하지 않을 경우에는 생성된 cron ID와 종목코드를 확인한 뒤 작업을 제거하고,
목록에서 사라졌는지 확인합니다.

# 분할매수 규칙 백테스트하기 (8.3)

## 1. 준비물 대조 (Ada)

새 Ada 세션에서 시작합니다. 설치를 반복하지 않고, 있는 것을 확인만 합니다.

```text
백테스트 준비물을 확인해줘.
이 프로젝트의 설치 영수증과 수집 산출물에서 두 ETF 장기 일봉의
공통 기간과 검증 통과 여부를 읽고, Supabase finance 장부에
그 공통 기간의 일봉이 두 종목 모두 빠짐없이 있는지 대조해줘.
공통 기간 밖의 행은 문제 삼지 않아도 돼.
어긋나면 진행하지 말고 무엇이 다른지만 보고해줘.
```

## 2. 백테스트 실행 (Ada)

구간을 나누는 기준은 사람이 정합니다. 실행과 정리는 Ada에게 맡깁니다.

```text
backtest/rules.md 규칙을 backtest/backtest.py로 검증할 거야.
종목은 rules.md의 기본 종목 하나로 하고,
공통 기간을 2020년 말까지(보정)와 2021년 이후(평가)로 나눠줘.
보정 구간에서 익절 후보 5·8·12%를 비교하고,
평가 구간은 운영 후보 8% 그대로 한 번만 실행해줘.
결과 보고서는 artifacts/backtest/에 저장하고,
각 실행의 최종자산·일괄매수 대비·최대 낙폭·미청산 수량을 표로 정리해줘.
손 검산 통과 표시와 감사 도구 검사는 아직 하지 마.
그건 내가 직접 확인한 뒤에 따로 시킬 거야.
```

마지막 두 줄이 없으면 에이전트가 검산 표시와 감사까지 한 번에 끝내 버려, 사람 검산이
게이트로 작동하지 않습니다(WinServerWSL 리허설 실측).

결과를 읽은 뒤, 성격이 다른 구간에서도 같은 규칙을 확인합니다.

```text
같은 계약으로 세 구간을 따로 한 번씩만 더 돌려줘.
2020-04-01~2021-06-30, 2021-07-01~2022-10-31, 2023-01-01~2023-12-31.
익절은 8% 그대로 두고, 각 구간의 최종자산·일괄매수 대비·최대 낙폭·매도 사이클 수를
표로 정리해줘. 손 검산 표시와 감사는 하지 마.
```

## 3. 손 검산 (Ada + 사람)

숫자를 믿기 전에 한 사이클을 사람이 따라가 봅니다.

```text
보정 구간 보고서에서 첫 번째 매수-매도 사이클을 골라서
매수일·체결가·보유 수량·평단, 그리고 매도 판정일의 종가와
평단×1.08 값을 표로 뽑아줘. 내가 일봉 화면과 대조할 거야.
```

손으로 확인한 결과는 다음 단계의 감사에서 기록합니다.

## 4. 감사 도구 검사 (Ada)

```text
artifacts/backtest/의 평가 구간 보고서를 백테스트 감사 도구로 검사해줘.
valid, errors, warnings, decision_eligible만 보여줘.
```

경고에 손 검산 미확인이 남아 있을 것입니다. 도구는 우리가 검산했는지 모릅니다 —
경고는 값을 바꿔서가 아니라, 한 일을 기록해서 없앱니다.

```text
방금 사이클 손 검산이 스크립트와 일치했어.
같은 조건으로 평가 구간 보고서를 hand-check 통과 상태로 다시 저장하고,
다시 감사 도구로 검사해줘.
```

감사 통과는 "회의에 올릴 수 있다"까지입니다. 보고서 안에는 배당 미포함처럼
아직 확인하지 못한 항목이 경고로 남아 있어 문서 상태는 초안(draft)입니다.
회의에 올릴 수 있다와 채택해도 된다는 다른 말입니다.

## 5. 결정 기록 (사람)

운영값을 유지할지 바꿀지는 사람이 정합니다. `backtest/rules.md`의
파라미터 표 근거 칸에 오늘 결정과 이유를 직접 적습니다.
에이전트에게 이 파일을 고치게 하지 않습니다.

# 크론으로 매일 자동 주문 돌리기 (8.4)

## 1. 스타터 최신화와 준비 확인 (Sam)

```bash
cd ~/.hermes/workspace/magma-finance-lab
git status --short
git pull --ff-only
```

git pull이 로컬 수정으로 중단되면 병합을 에이전트에 맡깁니다.

```text
스타터 저장소를 최신본으로 받으려는데 git pull이 로컬 수정 때문에 중단됐어.
이 수정은 백테스트 뒤에 내가 직접 남긴 검사 기록이라 잃으면 안 돼.
내 기록을 로컬 커밋으로 남긴 다음 최신본과 병합해줘.
겹치는 부분이 있으면 내 기록과 갱신된 문구를 모두 보존하는 쪽으로 정리하고,
원격으로 푸시는 하지 마. 끝나면 그 파일에 두 내용이 다 있는지 보여줘.
```

새 Sam 세션에서 준비 상태를 확인합니다. 설치를 반복하지 않습니다.

```text
주문을 시작하기 전에 준비 상태를 확인해줘.
키움 CLI 경로와 모의계좌 인증, doctor 결과를 요약하고,
guardrails 폴더의 한도 문서와 승인 템플릿이 있는지 확인해줘.
비밀값과 계좌번호는 출력하지 마.
```

## 2. 직무 분리 확인 (Ada · Noah)

Ada와 Noah 각각 새 세션에서 물어봅니다. "없다"가 정답입니다.

```text
네가 지금 쓸 수 있는 도구 목록에 주문이나 증권사 호출이 가능한 것이 있는지 알려줘.
```

권한을 주지 않은 일은 실수로도 못 합니다. 판단 코드는 주문을 못 내고,
집행 담당은 조건표를 해석하지 않으며, 보고 담당은 데이터베이스에 닿지 못합니다.

## 3. Sam에 루프 규칙 설치

같은 한 줄 지시가 동작하려면 규칙이 미리 심어져 있어야 합니다.
Sam 세션에 아래 요청과 두 규칙 블록을 한 번에 붙여넣습니다.

```text
아래 두 규칙을 너의 SOUL에 추가해줘.
같은 제목의 규칙이 이미 있으면 추가하지 말고 이미 있다고만 알려줘.

## 자산운용팀 — 승인 요청 처리 규칙

주문처럼 되돌릴 수 없는 일은 내가 판단만 하고, 집행 전에 반드시 사람의 승인을 받는다.

승인 대기 중인 카드가 있을 때, 슬랙에서 나를 멘션해 "승인" 이나 "승인해" 라고만 말하면 아래를 그 순서대로 수행한다. 매번 무엇을 할지 되묻지 않는다.

1. 승인 대기(blocked) 카드를 찾는다
2. 대상 카드가 하나면 그 카드로, 여럿이면 어느 것인지 되묻는다
3. 카드에 승인 사실을 코멘트로 남긴다. 누가 언제 승인했는지가 기록에 남아야 한다
4. 카드를 다시 실행 대기로 바꾼다 (unblock)
5. 즉시 한 장만 디스패치한다 (dispatch --max 1)
6. 처리 결과를 한 줄로 보고한다 (카드 ID·상태)

"보류" 나 "거부" 라고 하면 카드에 그 사유를 코멘트로 남기고 blocked 상태를 유지한다. 디스패치하지 않는다.

주의할 것:
- 승인 없이 주문을 집행하지 않는다. 사람이 명시적으로 승인한 카드만 진행한다
- 승인 한 번은 그 카드 한 건에만 유효하다. 다음 주문은 다시 승인을 받는다
- 카드 내용이 승인 시점과 달라졌으면 진행하지 말고 다시 확인받는다

## 자산운용팀 — 매일 판단 루프

"판단 루프", "오늘의 판단" 이라고 하면 아래를 뜻한다. 매번 무엇을 할지 되묻지 않는다.

작업 폴더는 ~/.hermes/workspace/magma-finance-lab 다. 규칙과 판정 기준은 그 폴더의 backtest/rules.md 에 있다.

1. broker/decide.py 를 실행해 오늘의 판단을 기안한다
   - 판단 결과는 state/decisions/<날짜>.json 에 남는다. 이 파일이 판단의 정본이다
2. 판단이 매수나 매도면 Supabase MCP 로 finance.orders 에 그 판단을 넣는다
   - status 는 drafted 다. 승인 전이므로 집행된 주문이 아니다
   - 판단 파일의 값을 그대로 옮긴다. 숫자를 다시 계산하지 않는다
3. 그 내용으로 칸반 카드를 만들고 blocked 로 둔다
   - 카드 본문은 guardrails/approval-template.md 형식을 따른다
4. 승인 요청을 슬랙 채널로 보낸다
5. 판단이 없으면(장 미개장·오늘 이미 판정함·수량 상한 도달) 카드를 만들지 않고 그 사실만 한 줄로 알린다
6. 중간에 실패하면 실패한 사실과 오류 내용을 슬랙으로 알린다. 조용히 끝내지 않는다

예약해 달라고 하면 위 내용을 cron 잡으로 등록한다. 등록 후 예약 내용을 한 줄로 요약해 알린다.

주의할 것:
- 주문은 어떤 경우에도 자동으로 집행하지 않는다. 승인을 받은 뒤에만 집행한다 (위 승인 요청 처리 규칙을 따른다)
- 크론으로 실행될 때도 마찬가지다. 사람이 없는 시간에 돈다고 해서 승인을 건너뛰지 않는다
- 판단 근거(현재가·보유 수량·평단)를 카드에 남긴다. 숫자 없이 결론만 적지 않는다
```

설치를 확인합니다.

```text
자산운용팀 승인 규칙과 판단 루프 규칙이 SOUL에 있는지 확인하고 각각 한 줄로 요약해줘.
```

## 4. 판단 루프 실행 (Slack)

슬랙 채널에서 Sam을 멘션합니다. 에이전트가 대신 보낸 메시지는 지시로 처리되지 않으므로
반드시 사람이 직접 입력합니다.

```text
@Sam 판단 루프를 지금 한 번 돌려줘
```

기안·카드·승인 요청 8항목이 슬랙에 올라옵니다.

## 5. 보류와 승인 (Slack)

거부부터 확인합니다. 안 된다고 했을 때 정말 멈추는가가 승인 게이트의 본체입니다.

```text
@Sam 보류. 시세를 더 보고 결정할게.
```

카드는 승인 대기로 남고 주문은 나가지 않습니다. 이제 승인합니다.

```text
@Sam 승인
```

승인 요청에는 만료 시각이 있습니다. 만료가 지나면 승인해도 집행되지 않고
새 기안을 요청하게 됩니다.

집행 직전 재검사에서 승인이 무효가 되면 회복하는 길은 둘입니다.
기안은 그대로 두고 허용 범위를 정해 다시 재검사하거나,

```text
한도 문서에 집행 재검사 허용 범위를 추가하고 확정했어.
오늘 기안을 그 기준으로 다시 재검사해서, 범위 안이면 승인 요청을 다시 올려줘.
새 기안은 만들지 마.
```

기안 자체를 새로 만듭니다. 이때도 오늘 판정 기록은 지우지 않고 보관합니다.

```text
오늘 판정 기록은 지우지 말고 보관 폴더로 옮겨 두고,
판단 루프를 처음부터 다시 돌려줘. 승인 요청이 오면 바로 확인할게.
```

## 6. 집행 결과 대사 (Sam)

```text
방금 집행한 주문의 체결과 잔고를 대조해서
기안·주문·체결·보유가 서로 맞는지 요약해줘.
비밀값과 계좌번호는 출력하지 마.
```

대사한 결과를 기록에 남깁니다. 이 기록이 있어야 Noah의 하루 보고가 실제와 일치합니다.

```text
방금 집행한 주문의 접수·체결·잔고 결과를 오늘 판단 기록과 카드에 남겨줘.
```

## 7. 중복 방지 확인 (Slack)

집행까지 끝난 상태에서 중복 방지를 확인해 봅니다.

```text
@Sam 판단 루프 한 번 더 돌려줘
```

오늘 이미 판정했다는 응답이 정답입니다. 크론이 두 번 돌거나 사람이 한 번 더 시켜도
같은 날 두 번 기안하지 않습니다.

## 8. 하루 보고 (Noah)

Noah 새 세션에서 요청합니다. Noah는 데이터베이스에 접근할 수 없어
판단 파일과 카드 기록만으로 씁니다.

```text
오늘 판단 파일과 칸반 카드 기록을 읽고, 무엇을 왜 기안했고
승인과 집행이 어떻게 됐는지 세 줄로 정리해 슬랙 채널에 올려줘.
```

## 9. 매일 아침 예약 (Slack)

```text
@Sam 판단 루프를 평일 아침 8시 40분에 예약해줘
```

```text
@Sam 예약된 작업 목록 보여줘
```

지속 운영하지 않을 때는 예약을 지웁니다.

```text
@Sam 방금 만든 판단 루프 예약을 지워줘
```

# 여러 에이전트로 투자위원회 열기 (8.5)

## 1. 회의 도구 설치와 확인

의장 프로필(Sophie)에만 설치합니다. 패널 프로필에는 필요 없습니다.
아래 명령은 스타터 저장소 루트에서 실행합니다.

```bash
hermes -p sophie plugins install dandacompany/hermes-council --enable
hermes -p sophie plugins list
```

설치 명령에는 버전 고정 플래그가 없습니다. 목록의 `council` 행에서 버전이 `0.13.0` 이상이고
enabled인지 확인하는 것이 정식 절차입니다.

설치할 때는 저장소 경로(`dandacompany/hermes-council`)를, 제거하거나 설정을 바꿀 때는
플러그인 이름(`council`)을 씁니다. 헷갈리기 쉽지만 Hermes 플러그인의 정상 규약입니다.

```bash
hermes -p sophie council doctor
```

네 항목이 모두 통과하면 준비된 것입니다. 비대화형 셸(SSH 등)에서 실패하면
`export PATH="$HOME/.local/bin:$PATH"`를 실행한 뒤 다시 시도합니다.

## 2. 안건 문서 준비 (사람)

안건 문서는 사람이 확정하는 입력입니다. 템플릿을 복사해 자기 산출물의 값으로 채웁니다.

```bash
ls briefs/
cp briefs/council-brief-template.md briefs/council-brief.md
```

`ls`로 템플릿이 있고 `council-brief.md`는 아직 없는 것을 확인한 뒤 복사합니다.
대상 파일이 이미 있으면 덮어쓰지 말고 다른 이름으로 옮긴 뒤 복사합니다.

8.3의 백테스트 결과, ETF 분석 상태, 8.4의 포지션·한도를 각각 그 문서에서 옮겨 적습니다.
확인하지 못한 항목은 비웠다고 적습니다. 없는 수치를 채워 넣으면 회의가 그 위에서 논의합니다.
템플릿의 고지 문장은 지우지 않습니다.

채우는 길은 셋입니다. 위처럼 빈 템플릿을 복사해 직접 채우거나, 아래 받아쓰기 프롬프트로
값을 불러 주거나, 완성 예시를 복사해 값만 자기 것으로 바꿉니다.

```bash
cp briefs/council-brief-example.md briefs/council-brief.md
```

예시의 수치는 MAGMA 실습 데이터입니다. 자기 백테스트 결과와 잔고로 바꾸지 않으면
회의가 남의 숫자를 심의하게 됩니다.

받아 적는 일은 Sophie에게 맡길 수 있습니다. 무엇을 근거로 넣을지는 사람이 정합니다.
받아 적는 노동만 맡기는 것이며, 채워진 값은 사람이 화면에서 확인합니다.

```text
@Sophie briefs/council-brief-template.md 를 복사해서 briefs/council-brief.md 를 만들고, 내가 부르는 값으로 대괄호를 채워줘.
없는 항목은 비었다고 그대로 적고, 값을 추정하지 마. 고지 문장과 상단 안내 문단은 지우지 마.

- 날짜: 2026-08-13
- 제목: 다음 사이클 심의 — KODEX 200 규칙과 2종째 ETF 후보
- inputs: ETFAnalysisSnapshot 미완성 · BacktestReport draft(게이트 2건 잔존) · 현재 포지션·한도
- 오늘 결정할 것: KODEX 200 분할매수 규칙과 2종째 ETF 후보를 매일 아침 도는 주문 루프의 정식 입력으로 승격할지, 검증 게이트가 끝날 때까지 보류할지 정한다.
- BacktestReport 날짜: 2026-08-11
- 초기자금 2,000,000원, 수수료 1.5bps, 슬리피지 2bps
- 보정 구간 2014-05-19~2020-12-30, 익절 후보 5·8·12% 중 8%가 6.5508%로 최고, 후보 간 차이 0.66%p
  → 이 차이로는 값을 고를 수 없다. 8% 유지는 성과가 아니라 운영 결정이다.
- 평가 구간 2021-01-04~2026-08-07, 전략 +19.98%, 일괄매수 +146.18%, 초과수익 -126.20%p, 상한 10주에 묶인 미청산 포지션이 기말까지 남음
- 검증 상태: 손 검산 통과, 감사 도구 검사 통과, 잔존 게이트는 수정주가 일관성과 배당 포함 기준 두 가지, 그래서 문서 상태는 draft
- ETFAnalysisSnapshot: 미완성. 가격·추세는 일봉만 있고 수정주가 의미 재확인 필요, 상품 구조·기초지수 가치는 미수집, 데이터 품질은 배당 제외에 발표 시점과 원문 해시 미완성
- 현재 포지션(2026-08-12): 모의계좌 KODEX 200 3주, 평단 100,885원
- 한도: status confirmed, 1회 2주, 하루 기안 1회, 보유 상한 10주, 집행 재검사 허용 범위 ±1.5%
- 주문 루프에서 검증한 것: 승인 만료, 집행 직전 재검사, 잔고 대사, 하루 1회 중복 방지, 평일 08시 40분 크론 예약
- 아직 확인하지 못한 것: 2종째 KRX 상장 미국지수 추종 ETF의 종목과 키움 모의 지원 여부 / 같은 데이터 계약과 비용으로 수행한 2종째 백테스트 / 잔존 게이트 2건
- 패널 질문: Ada는 이 BacktestReport를 draft에서 validated로 올릴 수 있는지, Oliver는 상품 구조가 비어 있는 상태에서 두 ETF를 비교할 수 있는지, Noah는 장기 하락·배당 제외·수정주가 오류·체결 괴리에서 이 결론이 어떻게 깨지는지, Sophie는 보류와 추가 검증을 구분하고 사람에게 넘길 선택지를 정리했는지
- 권장 기본 결론: 추가 검증. 벤치마크에 크게 뒤졌고 검증 게이트가 두 개 남았다.

다 채운 뒤에는 파일을 열어서 대괄호가 남아 있지 않은지 보여줘.
```

## 3. 회의 열기

여기서부터는 터미널 명령이 아니라 Sophie 세션에 말로 시킵니다.
먼저 예행으로 어떻게 열릴지만 확인합니다.

```text
~/.hermes/workspace/magma-finance-lab/briefs/council-brief.md 를 안건으로 투자위원회를 열어줘.
패널은 에이다·올리버·노아, 의장은 너야. 한 명씩 순서대로 말하게 하고 최대 세 턴까지만.
관점은 이렇게 나눠줘.
- 에이다: 데이터가 말하는 것 — 안건 문서의 시세·지표·백테스트 결과와 그 검증 상태
- 올리버: 상품 성격과 위험 — 안건 문서에 적힌 상품 구조와 비어 있는 항목
- 노아: 반대편에 서기 — 사지 말아야 할 이유를 찾고 다른 패널 발언을 반박
회의 중에 밖에서 새 자료를 찾지는 마.
결론을 내기 전에 반드시 나한테 물어봐.
먼저 예행으로 어떻게 열릴지만 보여줘.
```

관점은 패널 셋에게만 배정합니다. 의장은 관점을 받지 않습니다.
세 관점 모두 안건 문서 안에서만 근거를 찾게 적혀 있습니다. 회의 중에 각자 밖에서 자료를
찾아오면 패널마다 근거의 시점이 달라져, 같은 문서를 읽고 논의한다는 전제가 깨집니다.

"결론을 내기 전에 반드시 나한테 물어봐"가 사람 결정 게이트를 여는 지시입니다.
이 한 줄이 없으면 회의가 사람을 부르지 않고 결론까지 가 버립니다.

예행 결과에 역할 셋과 안건 문서, 사람 결정 게이트가 잡혔으면 그대로 엽니다.

```text
좋아, 이대로 열어줘.
```

안건 문서는 경로로 넘어갑니다. 에이전트가 파일을 읽어 회의에 붙입니다.

## 4. 회의 관찰

```text
회의 상태 알려줘. 지금 어느 발언까지 왔는지, 카드가 어떻게 도는지 짧게.
```

순차 회의라 패널이 한 명씩 앞 발언을 읽고 말합니다. 사람 결정 게이트를 켰기 때문에
결론 직전에 멈추고 사람 결정 대기 상태가 됩니다.

회의 이름을 따로 적어 둘 필요는 없습니다. 이어지는 요청은 같은 세션에서 하므로
에이전트가 어느 회의인지 기억합니다.

## 5. 사람 결정

```text
사람 결정 대기 상태인지 확인하고, 의장이 올린 질문과 선택지를 그대로 보여줘.
```

선택지의 근거를 회의록과 요약에서 읽고 결정합니다. 화면에 표시된 선택지 문구를
그대로 쓰고, 뒤에 자신의 이유를 한 문장 붙입니다.

```text
<화면의 선택지 문구>로 결정할게. 이유는 <이유 한 문장>이야.
```

결론은 채택·보류·추가 검증 중 하나입니다. 근거가 부족하면 보류가 정식 결론입니다.
결정을 미루는 것이 아니라, 지금 증거로는 여기까지라고 적는 것입니다.

## 6. 산출물 확인

```text
회의 보고서를 내보내줘. reports/council 폴더에도 사본을 남겨줘.
```

요약·결론 보고서·결정 기록·전체 회의록 네 종류가 나옵니다. 회의록에는 누가 무엇을
근거로 말했고 어디서 반박이 나왔는지가 남습니다.

이 결정 기록은 사람이 새 주문 기안을 검토해 만들 때 참고하는 입력입니다.
다음 아침 루프에 자동으로 반영되지 않습니다.
