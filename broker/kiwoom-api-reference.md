...
**실측 (2026-08-06 11:44, 정규장)** — 같은 요청이 정상 접수·체결됐다.
...
| --- | --- |
| 주문번호 | **** 필드 |
| 체결 | 시장가는 **즉시 체결**. 별도 체결 조회 없이 잔고에 반영 |
| 잔고 반영 | **즉시** (주문 직후 조회에 이미 반영) |
| 취소 | 즉시 체결돼 취소 대상 없음.  은 미체결 주문에만 |

> ⚠️ **잔고 응답의 종목코드에는  접두가 붙는다** — 주문·시세는 , 잔고는 . 적재·대사할 때 정규화한다.
**실측 (2026-08-06 11:44, 정규장)** — 같은 요청이 정상 접수·체결됐다.

```json
{"ord_no":"0096196","dmst_stex_tp":"KRX","return_code":0,"return_msg":"모의투자 매수주문완료"}
```
...
| 체결 | 시장가는 **즉시 체결**. 별도 체결 조회 없이 잔고에 반영 |
| 잔고 반영 | **즉시** (주문 직후 조회에 이미 반영) |
| 취소 | 즉시 체결돼 취소 대상 없음. `kt10003` 은 미체결 주문에만 |

> ⚠️ **잔고 응답의 종목코드에는 `A` 접두가 붙는다** — 주문·시세는 `069500`, 잔고는 `A069500`. 적재·대사할 때 정규화한다.

> ⏳ 지정가(`trde_tp: 0`) 주문과 미체결 주문 취소(`kt10003`)는 여전히 미검증.
...
- 조건검색
- 미국 주식 TR
- 지정가 주문()과 미체결 주문 취소() — 시장가 즉시 체결만 확인됨
